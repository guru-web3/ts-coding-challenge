...
interface TopicContext {
  ...
  topicId?: any; // Using 'any' as TopicId isn't clearly imported in original code
 ...
 */
async function createAccount(privateKey: PrivateKey, initialBalance: Hbar): Promise<[any, AccountId | null]> {
  try {
 ...
 */
async function verifyAccountBalance(accountId: AccountId, expectedMinimum: number): Promise<number> {
  try {
...
// Step definitions
Given(/^a first account with more than (\d+) hbars$/, async function(this: TopicContext, expectedBalance: number) {
  try {
    ...
    console.error(`Error setting up first account: ${(error as Error).message}`);
    throw error;
  }
});

Given(/^A second account with more than (\d+) hbars$/, async function(this: TopicContext, expectedBalance: number) {
  try {
    ...
    const [accountStatus, secondAccountId] = await createAccount(secondAccountKey, initialBalance);
    ...
    // Log account details
    console.log(`Second account created with ID: ${secondAccountId.toString()} and initial balance of ${initialBalance.toString()}`);
    
    // Verify the balance
    await verifyAccountBalance(secondAccountId, expectedBalance);
  } catch (error: unknown) {
    console.error(`Error setting up second account: ${(error as Error).message}`);
    throw error;
  }
});

Given(/^A (\d+) of (\d+) threshold key with the first and second account$/, async function(this: TopicContext, threshold: number, totalKeys: number) {
  try {
    // Validate inputs
    if (totalKeys !== 2) {
    ...
    console.log(`Threshold key ${threshold}/${totalKeys} created successfully`);
  } catch (error: unknown) {
    console.error(`Error creating threshold key: ${(error as Error).message}`);
    throw error;
  }
});

When(/^A topic is created with the memo "([^"]*)" with the first account as the submit key$/, async function(this: TopicContext, memo: string) {
  try {
    if (!this.privKey) {
    ...
    console.log(`Created topic with ID: ${this.topicId.toString()} and memo: "${memo}"`);
  } catch (error: unknown) {
    console.error(`Error creating topic with first account: ${(error as Error).message}`);
    throw error;
  }
});

When(/^A topic is created with the memo "([^"]*)" with the threshold key as the submit key$/, async function(this: TopicContext, memo: string) {
  try {
    ...
    console.log(`Topic created with ID: ${this.topicId.toString()} and memo: "${memo}"`);
  } catch (error: unknown) {
    console.error(`Error creating topic with threshold key: ${(error as Error).message}`);
    throw error;
  }
});

When(/^The message "([^"]*)" is published to the topic$/, async function(this: TopicContext, message: string) {
  try {
    if (!this.topicId) {
...
Then(/^The message "([^"]*)" is received by the topic and can be printed to the console$/, async function(this: TopicContext, expectedMessage: string) {
  ...
  try {
      ...
      const subscription = new TopicMessageQuery()
        ...
        .subscribe(
          ...
          (message: TopicMessage) => {
            ...
            console.log(`${message.consensusTimestamp.toDate()} Received: ${receivedMsg}`);
    ...
    console.log(`Successfully received expected message: "${receivedMessage}"`);
